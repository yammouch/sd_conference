...
The vertical axes is the sum of squared Euclidean distance between
$\hat{\bf y}_i$ and ${\bf y}_i$ calculated as follows;
\begin{eqnarray}
\sum_i||\hat{\bf y}_i - {\bf y}_i||^2
\end{eqnarray}
where $||\hat{\bf y} - {\bf y}||^2 = \sum_j(\hat{y}_j - y_j)^2$.
48 pairs of an output vector $\hat{\bf y}_i$ and a target vector ${\bf y}_i$
were picked up from the training data set to calculate the formula above,
to plot Fig.\ \ref{fig:errors} and to confirm the convergence.
