...
\section{Deep Feedforward Network}
...
This section briefly introduces one of neural network architectures called
``deep feedforward network'' applied to this study, described in \cite{mit}.
Figure \ref{fig:deep_feedforward} shows a basic structure of
deep feedforward network.
It consists of some hidden layers and an output layer.
It generates preferred output vector $\hat{\bf y}$ based on
trainied parameter ${\bm \theta}_n$.
...
$n$ is an integer ranging from 1 to $N$ and
${\bf x}_0$ is an input vector.
$f_n$ and $a_n$ are an weight function and an activation function,
explained later.
Parameters ${\bm \theta}_n$ is trained
by stochastic gradient descent method (SGC) described below.
SGC updates ${\bm \theta_n}$ so that it reduces an error
between $\hat{\bf y}_n$ and ${\bf y}_n$,
where ${\bf y}$ is a preferred output for an input, ${\bf x}_0$.
Given an loss function $L$ which evaluates
the error between $\hat{\bf y}$ and ${\bf y}$,
${\bm \theta}_n$ is updated as follows:
\begin{eqnarray}
{\bm \theta}_n \leftarrow
(1-\lambda){\bm \theta}_n
- \epsilon\frac{\partial L}{\partial {\bm \theta}_n}
\end{eqnarray}
where $\lambda$ is weight decay to above overfitting,
...
as a function of $\hat{y_i}-y_i$, when $y_i$ is 0 and 1, respectively.
...
\begin{eqnarray}
L(\hat{\bf y}, {\bf y}) = - y_i \log y_i - (1-y_i) \log (1-y_i)
\end{eqnarray}
